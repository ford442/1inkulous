...
// "0.1.0" — built by hand so the string is static storage with no allocation,
...
constexpr const char* kVersionText = INKULOUS_STRINGIFY(0) "." INKULOUS_STRINGIFY(1) "." INKULOUS_STRINGIFY(0);

static_assert(kVersionMajor == 0 && kVersionMinor == 1 && kVersionPatch == 0,
...
std::int32_t Simulation::advance(double delta_ms) {
  ...
  while (accumulator_ms_ >= step_ms_) {
    ...
    ++step_count_;
    ++steps;
    // Simulation work for one step goes here.
  }
